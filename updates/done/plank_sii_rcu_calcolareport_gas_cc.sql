CREATE DEFINER=`plank`@`%` PROCEDURE `plank_sii_rcu_calcolareport_gas_cc`(IN var_idmassiveimport INT, IN var_periodo VARCHAR(6), IN var_particellaazienda VARCHAR(10))
BEGIN

DECLARE var_perc_consumo_rcu DOUBLE;

UPDATE plank_sii_rcu_dettagli_gas SET idcontratto=NULL, periodo=var_periodo WHERE idmassiveimport=var_idmassiveimport;
...
DROP TEMPORARY TABLE IF EXISTS _sii_rcu_dettagli_gas;
CREATE TEMPORARY TABLE _sii_rcu_dettagli_gas
SELECT 
id, C.idcontratto, COD_PDR, COD_REMI, TIPO_PDR, DISALIMENTABILITA, STATO_PDR, TIPO_SOSP, DATA_INIZIO_SOSP, DATA_FINE_SOSP, DATA_INIZIO_FOR, DATA_FINE_FOR, DATA_INIZIO_MAND, DATA_FINE_MAND, PIVA_UDD, RAGIONE_SOCIALE_UDD, PIVA_DD, RAGIONE_SOCIALE_DD, CF, PIVA, CF_STRANIERO, NOME, COGNOME, RAGIONE_SOCIALE_DENOMINAZIONE, EMAIL, TELEFONO, REFERENTE, REF_NOME, REF_COGNOME, REF_EMAIL, REF_TELEFONO, RESIDENZA, DATA_VAL_RES, AF_CF, AF_PIVA, AF_CF_STRANIERO, AF_NOME, AF_COGNOME, AF_RAGIONE_SOCIALE_DENOMINAZIONE, ALIQUOTA_IVA, ALIQUOTA_ACCISE, ADDIZ_REGIONALE, ALTRE_INFORMAZIONI, ACCESSO_UI, TIPO_FORNITURA, CODICE_UFFICIO, PAGAMENTO_IVA, SETT_MERCEOLOGICO, TRATTAMENTO, PREL_ANNUO_PREV, COD_PROF_PREL_STD, CAT_USO, CLASSE_PRELIEVO, ANNO_TERMICO, MATR_MIS, CLASSE_GRUPPO_MIS, TIPO_MIS, TELEGESTIONE, PRE_CONV, MATR_CONV, N_CIFRE_CONV, ANNO_FABB_CONV, DATA_INST_CONV, COEFF_CORR, PRESS_MISURA, ACC_MIS, N_CIFRE_MIS, ANNO_FABB_MIS, DATA_INST_MIS,
GRUPPO_MIS_INT, POT_MAX_RIC, POT_TOT_INST, MAX_PRELIEVO_ORA, BONUS, BS_DATA_INIZIO, BS_DATA_FINE, BS_DATA_RINNOVO, TIPO_BONUS, EROG_SERVIZIO_ENERG, SE_PIVA, SE_RAGIONE_SOCIALE_DENOMINAZIONE, SE_TELEFONO, SE_EMAIL, idmassiveimport, RCU.particellaazienda, periodo
FROM comm.plank_sii_rcu_dettagli_gas RCU
...
UPDATE comm.plank_sii_rcu_dettagli_gas RCU
INNER JOIN
(
	SELECT id, GROUP_CONCAT(idcontratto) as idcontratto,cod_pdr
    FROM _sii_rcu_dettagli_gas
    GROUP BY id, cod_pdr
) C on C.id=RCU.id
...
CT.infocontratto
FROM _sii_rcu_dettagli_gas RCU
...
SELECT E.id, E.idcontratto FROM _sii_rcu_dettagli_gas E
...
DELETE E FROM _sii_rcu_dettagli_gas E
...
WHERE RCU.idmassiveimport=var_idmassiveimport and REPLACE(RCU.PREL_ANNUO_PREV,",",".") REGEXP '^[0-9]+\\.?[0-9]*$' and RCU.PREL_ANNUO_PREV NOT LIKE ',%' 
	and ((IFNULL(C.gas_consumoprevisto,0)=0 and C.gas_consumoprevisto<>REPLACE(RCU.PREL_ANNUO_PREV,",",".")) or
		(var_perc_consumo_rcu>0 and ABS(REPLACE(RCU.PREL_ANNUO_PREV,",",".")-C.gas_consumoprevisto)/REPLACE(RCU.PREL_ANNUO_PREV,",",".")>=var_perc_consumo_rcu));
...
#UDD
INSERT INTO plank_sii_rcu_report_gas(idmassiveimport,idcontratto,idrcu,pdr,casistica,oldvalue,newvalue,processed,particellaazienda,nomecampo,nometabella,storicizzazione, decorrenza,infocontratto)
select var_idmassiveimport, RCU.idcontratto, RCU.id, RCU.COD_PDR, "UDD", CONCAT(A.densociale," - ",A.piva), CONCAT(RCU.RAGIONE_SOCIALE_UDD," - ",RIGHT(CONCAT("0000",RCU.PIVA_UDD),11)), 
...
LEFT JOIN (
    ...
    FROM plank_assegnazionefornitori_gas A
    INNER JOIN plank_contrattigrossisti_gas C on C.idcontratto=A.contrattofornitura
